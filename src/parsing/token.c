...
void	add_token(t_cmd_tab *cmd_tab, int lexem, char letter)
{
	...
	if (!token)
		return (set_error(MALLOC_FAIL, cmd_tab, "Failed to allocate token"));
	token->content = ft_calloc(sizeof(char), 2);
	if (!token->content)
		return (set_error(MALLOC_FAIL, cmd_tab, "Failed to alloc token data"));
	token->lexem = lexem;
	token->letter = letter;
	...
	if (!node)
		return (set_error(MALLOC_FAIL, cmd_tab, "Failed to alloc token node"));
